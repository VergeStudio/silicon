...
int main (void)
{
	...
	sffi_cif cif;
	sffi_type **struct_float1 = (sffi_type **)malloc(sizeof(sffi_type *) * (structElemNum + 1));
	sffi_type **struct_float2 = (sffi_type **)malloc(sizeof(sffi_type *) * (nestedStructElemNum + 1));
	sffi_type **args = (sffi_type **)malloc(sizeof(sffi_type *) * (argNum + 1));
	void **values = (void **)malloc(sizeof(void *) * (argNum + 1));
	sffi_type struct_float_type1, struct_float_type2;
	sffi_type *retType = &sffi_type_float;
	float arg1;
	float *arg2 = (float *)malloc(sizeof(stru_Nested_F));

	struct_float2[0] = &sffi_type_float;
	struct_float2[1] = &sffi_type_float;
	struct_float2[2] = NULL;

	struct_float_type2.size = 0;
	struct_float_type2.alignment = 0;
	struct_float_type2.type = SFFI_TYPE_STRUCT;
	struct_float_type2.elements = struct_float2;

	struct_float1[0] = &sffi_type_float;
	struct_float1[1] = &struct_float_type2;
	struct_float1[2] = NULL;

	struct_float_type1.size = 0;
	struct_float_type1.alignment = 0;
	struct_float_type1.type = SFFI_TYPE_STRUCT;
	...
	CHECK( sffi_prep_cif(&cif, SFFI_DEFAULT_ABI, 2, retType, args) == SFFI_OK);
	sffi_call(&cif, SFFI_FN(testNestedFloatStruct), &ts12_result, values);
