...
| 日期 | 提交 / 链接 | 概述 |
|------|------------|------|
| 2026-09-07 | `6d18bb9` | README 改为"协作与获取入口":① **删除**原仓库接入上半段(仓库地址块/leo-dev 分支/clone 命令/工蜂认证,即原 L1-38),代码来源改为一行"从仓库 `leo/dev` 拉取、细节见 `memory/repo.md`";② 新增「记忆与进度:通过乐享团队知识库获取」段——明确每轮前通过乐享连接器读取 `memory/`(MEMORY.md+主题)与 `progress/PROGRESS.md`,列出读取入口并指向 conventions 的「乐享 silicon 镜像·结构与条目 id」小节;协作纪律/镜像两处/同步要求保留。③ `memory/repo.md` 标注为"代码仓库接入细节的唯一权威来源"(README 不再重复);`memory/MEMORY.md` 主题索引表补 MEMORY.md 自身一行。README 不镜像乐享(乐享无 README 页);repo.md/MEMORY.md 已镜像同步乐享。 |
