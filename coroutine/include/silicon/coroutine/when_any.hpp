...
static auto make_when_any_task_return_void(awaitable a, std::atomic<bool> &first_completed, silicon::coroutine::event &notify)
        -> silicon::coroutine::task<void> {
    co_await static_cast<awaitable &&>(a);
    auto expected = false;
...
static auto make_when_any_controller_task_return_void(range_type awaitables, silicon::coroutine::event &notify)
        -> silicon::coroutine::detail::task_self_deleting {
    std::atomic<bool> first_completed{false};
