...
class acquire_operation {
  public:
    ...
    auto await_suspend(const std::coroutine_handle<> awaiting_coroutine) noexcept -> bool {
        ...
        detail::awaiter_list_push(m_semaphore.m_p->m_acquire_waiters, this);
        m_semaphore.m_p->m_mutex.unlock();
        return true;
    }
...
class semaphore {
  public:
    explicit semaphore(const std::ptrdiff_t starting_value)
        : m_p(std::make_unique<P>(starting_value)) {}
    ...
    [[nodiscard]] auto release() -> silicon::coroutine::task<void> {
        ...
        auto *waiter = detail::awaiter_list_pop(m_p->m_acquire_waiters);
        if(waiter != nullptr) {
            m_p->m_mutex.unlock();
    ...
    [[nodiscard]] auto shutdown() noexcept -> silicon::coroutine::task<void> {
        ...
        if(m_p->m_shutdown.compare_exchange_strong(expected, true, std::memory_order::release, std::memory_order::relaxed)) {
            auto *waiter = detail::awaiter_list_pop_all(m_p->m_acquire_waiters);
            lock.unlock();
  ...
  private:
    ...
    class P {
      public:
        explicit P(const std::ptrdiff_t starting_value): m_counter(starting_value) {}
