...
export module silicon.coroutine:scheduler;
...
import :poll;
import :pool;
import :task; // 原经 pool.hpp 传递获得，分区 import 无传递性，须显式引入
import :when_any; // scheduler::schedule 超时重载使用
...
class scheduler {
  ...
  public:
     ...
     */
    auto spawn_detached(silicon::coroutine::task<void> &&task) -> bool;
     ...
     */
    auto spawn_joinable(silicon::coroutine::task<void> &&task) -> silicon::coroutine::task<void>;
...
// 接口；scheduler 即该接口的唯一实现（含 virtual 析构）。此别名补全缺失的接口名。
