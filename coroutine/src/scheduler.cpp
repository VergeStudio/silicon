...
namespace detail {
static auto
make_spawned_joinable_wait_task(std::unique_ptr<silicon::coroutine::task_group<silicon::coroutine::scheduler>> group_ptr) -> silicon::coroutine::task<void> {
    co_await *group_ptr;
    co_return;
}
...
scheduler::scheduler(options &&opts, private_constructor)
    ...
    if(!m_io_notifier.watch(m_shutdown_pipe.read_fd(), silicon::coroutine::poll_op::read, const_cast<void *>(m_shutdown_ptr), true)) {
        throw std::runtime_error("Failed to register m_shutdown_pipe.read_fd() for read events.");
    }

    if(!m_io_notifier.watch(m_schedule_pipe.read_fd(), silicon::coroutine::poll_op::read, const_cast<void *>(m_schedule_ptr), true)) {
    ...
    if(m_opts.execution_strategy == execution_strategy_t::process_tasks_on_thread_pool) {
        m_thread_pool = pool::make_unique(std::move(m_opts.pool));
    }
}
...
auto scheduler::spawn_detached(silicon::coroutine::task<void> &&task) -> bool {
...
auto scheduler::spawn_joinable(silicon::coroutine::task<void> &&task) -> silicon::coroutine::task<void> {
    auto group_ptr = std::make_unique<silicon::coroutine::task_group<silicon::coroutine::scheduler>>(this, std::move(task));
    return detail::make_spawned_joinable_wait_task(std::move(group_ptr));
}

auto scheduler::schedule_at(time_point time) -> silicon::coroutine::task<void> {
    return yield_until(time);
}

auto scheduler::yield_until(time_point time) -> silicon::coroutine::task<void> {
    auto now = clock::now();
...
auto scheduler::yield_for_internal(std::chrono::nanoseconds amount) -> silicon::coroutine::task<void> {
    if(amount <= 0ms) {
        co_await schedule();
    } else {
