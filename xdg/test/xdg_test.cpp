...
TEST_CASE("xdg base dirs are non-empty on current platform") {
    CHECK_FALSE(HomeDir().empty());
    CHECK_FALSE(ConfigHome().empty());
    CHECK_FALSE(DataHome().empty());
    CHECK_FALSE(CacheHome().empty());
    CHECK_FALSE(StateHome().empty());
}
