...
namespace detail {
static auto
make_spawned_joinable_wait_task(std::unique_ptr<silicon::coroutine::task_group<silicon::scheduler::io_scheduler>> group_ptr) -> silicon::coroutine::task<void> {
    co_await *group_ptr;
    co_return;
}
...
auto io_scheduler::spawn_detached(silicon::coroutine::task<void> &&task) -> bool {
    m_p->m_size.fetch_add(1, std::memory_order::release);
    auto wrapper_task = silicon::coroutine::detail::make_task_self_deleting(std::move(task));
    wrapper_task.promise().user_final_suspend([this]() -> void { m_p->m_size.fetch_sub(1, std::memory_order::release); });
    return resume(wrapper_task.handle());
}

auto io_scheduler::spawn_joinable(silicon::coroutine::task<void> &&task) -> silicon::coroutine::task<void> {
    auto group_ptr = std::make_unique<silicon::coroutine::task_group<silicon::scheduler::io_scheduler>>(this, std::move(task));
    return detail::make_spawned_joinable_wait_task(std::move(group_ptr));
}

auto io_scheduler::schedule_at(time_point time) -> silicon::coroutine::task<void> {
    return yield_until(time);
}

auto io_scheduler::yield_until(time_point time) -> silicon::coroutine::task<void> {
    auto now = clock::now();
...
auto io_scheduler::yield_for_internal(std::chrono::nanoseconds amount) -> silicon::coroutine::task<void> {
    if(amount <= 0ms) {
        co_await schedule();
    } else {
