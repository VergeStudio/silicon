...
- 维护入口：`/workspace/silicon/MEMORY.md`（仓库根单文件 = **权威副本**）
- 更新约定：见仓库根 `MEMORY.md` 头部 + 项目资产根 `README.md`（"团队协作要求"+"每轮进行前"两节）。
- 资产侧视图：项目资产 `memory/` 目录按主题拆分本文件内容（索引 `memory/MEMORY.md` + 主题文件 repo/build/verify/adr/conventions/collab），与本文档保持内容一致。
...
## 6. 团队协作要求（写入项目资产根 README.md）

每位成员须遵循「**轮前读取 → 轮中更新 → 轮后同步**」闭环（见项目资产根 README.md）：

- **每轮开始前**：先读取并同步记忆与进度——记忆入口 `memory/MEMORY.md`（按需读 repo/build/verify/adr/conventions/collab 主题），进度 `progress/PROGRESS.md`；动手前把本轮目标/预期写入对应文件。
- **每轮修改后**：同步两份工作文档——
  1. **进度 PROGRESS.md**（仓库根，随代码 commit+push `leo/dev`；资产侧覆盖 `progress/PROGRESS.md`）——把本轮进展、提交 hash、待办登记进"变更日志"，让下一位成员可续接。
  2. **记忆 MEMORY.md**（仓库根单文件权威副本；资产侧按主题覆盖 `memory/` 下对应文件 + 维护索引）——把本轮产生的稳定知识（新约定/新架构要点/新踩坑）沉淀进对应章节。
- 资产侧为**跨沙箱权威副本**，便于成员在不 clone 仓库时按主题读到最新记忆与进度。
- 若本轮无新进度/稳定知识，可在相应文件注明"本轮无变更"，保持时间线可追溯。

> 资产目录结构（详见项目资产根 README.md）：`README.md`（本说明）+ `memory/`（MEMORY.md 索引 + 6 个主题文件）+ `progress/PROGRESS.md`。
