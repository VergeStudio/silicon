...
auto task_event::is_set() const noexcept -> bool {
    return m_state.load(std::memory_order::acquire) == this;
}

auto task_event::awaiter::await_suspend(std::coroutine_handle<> awaiting_coroutine) noexcept -> bool {
...
auto task_event::set() noexcept -> void {
...
auto task_event::reset() noexcept -> void {
    void *expected = this;
