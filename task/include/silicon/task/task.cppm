...
export module silicon.task;

export import :config;

export namespace silicon::task {
...
// ---------------------------------------------------------------------------
//
// NOTE: 这些内部辅助类型必须与公共头 include/silicon/task/detail/
// task_self_deleting.hpp 保持同一命名空间（silicon::task::detail），
...
auto make_task_self_deleting(silicon::task::task<void> user_task) -> task_self_deleting;
...
class task_group {
  public:
    ...
    explicit task_group(executor_type *executor, silicon::task::task<void> &&task)
        : task_group(executor) {
        (void)start(std::forward<silicon::task::task<void>>(task));
    }
    ...
    explicit task_group(std::unique_ptr<executor_type> &executor, silicon::task::task<void> &&task)
        : task_group(executor.get(), std::forward<silicon::task::task<void>>(task)) {}
    ...
    [[nodiscard]] auto start(silicon::task::task<void> &&task) -> bool {
