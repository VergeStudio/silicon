...
- **资产同步已获"自动覆盖、免逐一确认"授权(2026-09-07 起)**:当轮成员在本轮修改完成后**自动**将改动的
  记忆与进度覆盖上传到资产对应条目,**无需再逐次询问**;常规镜像同步一律自动完成并下载+MD5 校验一致。
  仅遇无法自行解决的异常(无 `can_delete` 需删字面 `.md` 残留、覆盖意外新增重复需他人介入)才上报沟通。
