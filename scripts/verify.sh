...
#
# 用法:
#   scripts/verify.sh            # 全量 clean 重建 + 测试 + dylib 符号检查 + warning 统计
...
echo "==> [verify] pwd=$ROOT"

# --- 构建 ---
if [[ $NOBUILD -eq 0 ]]; then
  if [[ $INC -eq 0 ]]; then
    echo "==> [verify] xmake clean -a"
    xmake clean -a || { echo "!! clean failed"; exit 1; }
  fi
  # clean 后首跑会因 config 模板生成顺序报一次良性 warning:
...
# --- 定位测试二进制与 dylib（直接找一个真实 *.test，取其所在目录，避免误匹配 .objs 等嵌套目录） ---
TEST_BIN=$(find build -type f -name '*.test' 2>/dev/null | head -1)
if [[ -z "$TEST_BIN" ]]; then
  echo "!! no *.test binaries found under build/"; exit 1
fi
TEST_DIR=$(dirname "$TEST_BIN")
echo "==> [verify] test dir=$TEST_DIR"
