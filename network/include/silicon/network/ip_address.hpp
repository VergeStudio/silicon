...
class ip_address {
  public:
    ...
    ip_address() = default;
    ip_address(std::span<const uint8_t> binary_address, domain_t domain = domain_t::ipv4): m_p(std::make_shared<P>()) {
        m_p->m_domain = domain;
        if(m_p->m_domain == domain_t::ipv4 && binary_address.size() > ipv4_len) {
