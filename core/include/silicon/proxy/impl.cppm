...
#        define PROD_UNREACHABLE() __builtin_unreachable()
#    else
#        define PROD_UNREACHABLE() ::std::terminate()
#    endif
#endif
...
using composite_t = recursive_reduction_t<
        reduction_traits<composition_reduction>::template type,
        T,
        Us...>;
...
template<class P, bool IsDirect, class D, qualifier_type Q, bool NE, class R, class... Args>
...
consteval void diagnose_proxiable_required_convention_not_implemented() {
    static_assert(overload_traits<O>::applicable && overload_traits<O>::template applicable_ptr<P, IsDirect, D>, "not proxiable due to a required convention not implemented");
}
    ...
    struct conv_meta<ProP, D, R(Args...) oq ne> {                                                              \
        ...
        constexpr explicit conv_meta(std::in_place_type_t<P>)                                                  \
            : invoke([](ProP pq self, Args... args) ne -> R {                                                  \
                  return reinterpret_invoke<P, D, R>(static_cast<ProP pq>(self), std::forward<Args>(args)...); \
              }) {}                                                                                            \
                                                                                                               \
...
template<class C>
    ...
    } && is_is_direct_well_formed<C>() && is_tuple_like_well_formed<typename C::overload_types>())
struct basic_conv_traits<C>
    : instantiated_t<basic_conv_traits_impl, typename C::overload_types, C> {};

template<class T>
struct a11y_traits_impl
    : std::conditional<std::is_nothrow_default_constructible_v<T> && std::is_trivially_copyable_v<T> && !std::is_final_v<T>, T, void> {};
template<class SFINAE, class T, class... Args>
struct a11y_traits: std::type_identity<void> {};
template<class T, class... Args>
struct a11y_traits<std::void_t<typename T::template accessor<Args...>>, T, Args...>
...
struct conv_traits_impl {
    ...
    template<class T>
    using accessor =
            accessor_t<typename C::dispatch_type, T, typename C::dispatch_type, substituted_overload_t<Os, F>...>;
...
consteval void diagnose_proxiable_required_reflection_not_implemented() {
    static_assert(is_reflector_well_formed<P, IsDirect, R>(), "not proxiable due to a required reflection not implemented");
}
...
struct refl_accessor_traits<R, F, false>
    : std::type_identity<
              accessor_t<typename R::reflector_type, proxy_indirect_accessor<F>, typename R::reflector_type>> {};
template<class R, class F>
    requires(R::is_direct)
struct refl_accessor_traits<R, F, true>
    : std::type_identity<accessor_t<typename R::reflector_type, proxy<F>, typename R::reflector_type>> {};
template<class R, class F, bool IsDirect>
using refl_accessor_t = typename refl_accessor_traits<R, F, IsDirect>::type;

template<class T>
concept pointer_like = (std::is_pointer_v<T> || requires { typename T::element_type; } || requires(T val) { *val; }) && requires { typename std::pointer_traits<T>::element_type; };
...
consteval void diagnose_proxiable_align_too_large() {
    static_assert(ActualAlign <= MaxAlign, "not proxiable due to alignment too large");
}
template<class P, class F, constraint_level RequiredCopyability>
consteval void diagnose_proxiable_insufficient_copyability() {
    static_assert(copyability_traits<P, RequiredCopyability>::applicable, "not proxiable due to insufficient copyability");
}
...
consteval void diagnose_proxiable_insufficient_relocatability() {
    static_assert(relocatability_traits<P, RequiredRelocatability>::applicable, "not proxiable due to insufficient relocatability");
}
...
consteval void diagnose_proxiable_insufficient_destructibility() {
    static_assert(destructibility_traits<P, RequiredDestructibility>::applicable, "not proxiable due to insufficient destructibility");
}
...
template<class F>
    ...
    } && is_facade_constraints_well_formed<F>() && is_tuple_like_well_formed<typename F::convention_types>() && instantiated_t<basic_facade_conv_traits_impl, typename F::convention_types>::applicable && is_tuple_like_well_formed<typename F::reflection_types>() && instantiated_t<basic_facade_refl_traits_impl, typename F::reflection_types>::applicable)
...
struct facade_refl_traits_impl {
    ...
    static constexpr bool refl_applicable_ptr =
            (is_reflector_well_formed<P, Rs::is_direct, typename Rs::reflector_type>() &&
             ...);
};
template<class F>
struct facade_traits
    ...
    using meta = composite_t<
            ...
            lifetime_meta_t<F, relocate_dispatch, void(proxy<F> &) && noexcept, void(proxy<F> &) &&, F::relocatability>,
    ...
    using indirect_accessor =
            composite_t<typename facade_traits::conv_indirect_accessor, typename facade_traits::refl_indirect_accessor>;
    using direct_accessor =
            composite_t<typename facade_traits::conv_direct_accessor, typename facade_traits::refl_direct_accessor>;
...
struct meta_ptr_traits_impl<composite_meta<conv_meta<ProP, D, O>, Ms...>>
    : std::type_identity<
              meta_ptr_direct_impl<composite_meta<conv_meta<ProP, D, O>, Ms...>, conv_meta<ProP, D, O>>> {};
template<class M>
struct meta_ptr_traits: std::type_identity<meta_ptr_indirect_impl<M>> {};
template<class M>
    requires(sizeof(M) <= sizeof(ptr_prototype) && alignof(M) <= alignof(ptr_prototype) && std::is_nothrow_default_constructible_v<M> && std::is_trivially_copyable_v<M>)
...
class proxy_indirect_accessor
  ...
  public:
    ...
    template<class D, class O, class... Args>
    friend details::ret_t<O> invoke(const proxy_indirect_accessor &p, Args &&...args) {
        return details::invoke_impl<D, O>(p, std::forward<Args>(args)...);
    }
    template<class D, class O, class... Args>
    friend details::ret_t<O> invoke(proxy_indirect_accessor &&p, Args &&...args) {
        return details::invoke_impl<D, O>(std::move(p), std::forward<Args>(args)...);
    }
    template<class D, class O, class... Args>
    friend details::ret_t<O> invoke(const proxy_indirect_accessor &&p, Args &&...args) {
        return details::invoke_impl<D, O>(std::move(p), std::forward<Args>(args)...);
...
class proxy: public details::facade_traits<F>::direct_accessor,
  ...
  public:
    ...
    constexpr proxy(P &&ptr) noexcept(
        ...
        requires(!details::specialization_of<std::decay_t<P>, proxy> && details::pointer_like<std::decay_t<P>> && std::is_constructible_v<std::decay_t<P>, P>)
    {
    ...
    constexpr explicit proxy(
            ...
            Args &&...args
                       ...
                       is_nothrow_constructible_v<
                               ...
                               Args...>)
        requires(details::pointer_like<P> && std::is_constructible_v<P, std::initializer_list<U> &, Args...>)
    {
  ...
  private:
    ...
    PRO4D_DEBUG(static inline void pro_symbol_guard(proxy &self, const proxy &cself) {
        self.operator->();
        *self;
        *std::move(self);
        cself.operator->();
...
struct converter {
    ...
    operator T() && noexcept(
        ...
        requires(std::is_invocable_r_v<T, F, std::in_place_type_t<T>> && !std::is_invocable_r_v<T, F, std::in_place_type_t<T &>> && !std::is_invocable_r_v<T, F, std::in_place_type_t<T &&>>)
    {
...
using add_tuple_t =
        recursive_reduction_t<reduction_traits<add_tuple_reduction>::template type, O, Is...>;
...
template<class C>
    requires(C::is_direct && std::is_same_v<typename C::dispatch_type, substitution_dispatch>)
struct observer_conv_traits<C>
    : std::type_identity<instantiated_t<observer_substitution_conv, typename C::overload_types>> {};
template<class C>
    requires(!C::is_direct)
...
using weak_substitution_conv =
        conv_impl<true, substitution_dispatch, typename weak_substitution_overload_traits<Os>::type...>;
...
struct observer_facade
    : details::facade_impl<
              details::instantiated_t<details::observer_conv_types, typename F::convention_types>,
...
struct weak_facade
    : details::facade_impl<
              details::instantiated_t<details::weak_conv_types, typename F::convention_types, F>,
              std::tuple<>,
              F::max_size,
              F::max_align,
              F::copyability,
...
struct strong_weak_compact_ptr_storage: strong_weak_compact_ptr_storage_base,
    ...
    explicit strong_weak_compact_ptr_storage(const Alloc &alloc, Args &&...args)
        : alloc_aware<Alloc>(alloc) {
        std::construct_at(reinterpret_cast<T *>(&value), std::forward<Args>(args)...);
    }
...
constexpr proxy<F> allocate_proxy_impl(const Alloc &alloc, Args &&...args) {
    if constexpr(proxiable<allocated_ptr<T, Alloc>, F>) {
        return proxy<F>{std::in_place_type<allocated_ptr<T, Alloc>>, alloc, std::forward<Args>(args)...};
    } else {
        return proxy<F>{std::in_place_type<compact_ptr<T, Alloc>>, alloc, std::forward<Args>(args)...};
    }
}
template<class F, class T, class... Args>
constexpr proxy<F> make_proxy_impl(Args &&...args) {
    if constexpr(proxiable<inplace_ptr<T>, F>) {
        return proxy<F>{std::in_place_type<inplace_ptr<T>>, std::in_place, std::forward<Args>(args)...};
    } else {
        return allocate_proxy_impl<F, T>(std::allocator<void>{}, std::forward<Args>(args)...);
    }
}
template<class F, class T, class Alloc, class... Args>
constexpr proxy<F> allocate_proxy_shared_impl(const Alloc &alloc, Args &&...args) {
    if constexpr(std::is_convertible_v<proxy<F>, weak_proxy<F>>) {
        return proxy<F>{std::in_place_type<strong_compact_ptr<T, Alloc>>, alloc, std::forward<Args>(args)...};
    } else {
        return proxy<F>{std::in_place_type<shared_compact_ptr<T, Alloc>>, alloc, std::forward<Args>(args)...};
    }
}
template<class F, class T, class... Args>
...
consteval constraint_level merge_constraint(constraint_level a, constraint_level b) {
    return a < b ? b : a;
}
...
struct basic_facade_builder {
    ...
    using add_indirect_convention = basic_facade_builder<
            details::add_conv_t<Cs, details::conv_impl<false, D, Os...>>,
            Rs,
            MaxSize,
            MaxAlign,
            Copyability,
    ...
    using add_direct_convention = basic_facade_builder<
            details::add_conv_t<Cs, details::conv_impl<true, D, Os...>>,
            Rs,
            MaxSize,
            MaxAlign,
            Copyability,
    ...
    using add_indirect_reflection = basic_facade_builder<
            Cs,
            details::add_tuple_t<Rs, details::refl_impl<false, R>>,
            MaxSize,
            MaxAlign,
            Copyability,
            Relocatability,
    ...
    using add_direct_reflection = basic_facade_builder<
            Cs,
            details::add_tuple_t<Rs, details::refl_impl<true, R>>,
            MaxSize,
            MaxAlign,
            Copyability,
            Relocatability,
    ...
    using restrict_layout =
            basic_facade_builder<Cs, Rs, details::merge_size(MaxSize, PtrSize), details::merge_size(MaxAlign, PtrAlign), Copyability, Relocatability, Destructibility>;
    template<constraint_level CL>
        requires(details::is_cl_well_formed(CL))
    using support_copy =
            basic_facade_builder<Cs, Rs, MaxSize, MaxAlign, details::merge_constraint(Copyability, CL), Relocatability, Destructibility>;
    template<constraint_level CL>
        requires(details::is_cl_well_formed(CL))
    using support_relocation =
            basic_facade_builder<Cs, Rs, MaxSize, MaxAlign, Copyability, details::merge_constraint(Relocatability, CL), Destructibility>;
    template<constraint_level CL>
        requires(details::is_cl_well_formed(CL))
    using support_destruction =
            basic_facade_builder<Cs, Rs, MaxSize, MaxAlign, Copyability, Relocatability, details::merge_constraint(Destructibility, CL)>;
...
#define PROD_LHS_BINARY_OP_DISPATCH_BODY_IMPL(...)         \
    ...
    PRO4D_DIRECT_FUNC_IMPL(std::forward<T>(self) __VA_ARGS__ std::forward<Arg>(arg))
...
#define PROD_LHS_OP_DISPATCH_IMPL(type, ...)                                              \
    ...
    struct operator_dispatch<#__VA_ARGS__, false> {                                       \
        PROD_LHS_##type##_OP_DISPATCH_BODY_IMPL(__VA_ARGS__) PRO4D_DEF_ACCESSOR_TEMPLATE( \
                MEM,                                                                      \
                PROD_DEF_LHS_##type##_OP_ACCESSOR,                                        \
...
#define PROD_DEF_RHS_OP_ACCESSOR(oq, pq, ne, ...)                                                                                                     \
    ...
    struct accessor<P, D, R(Arg) oq ne> {                                                                                                             \
        friend R operator __VA_ARGS__(Arg arg, P pq self) ne {                                                                                        \
            return invoke<D, R(Arg) oq ne>(static_cast<P pq>(self), std::forward<Arg>(arg));                                                          \
        }                                                                                                                                             \
        PRO4D_DEBUG(                                                                                                                                  \
                accessor() noexcept { std::ignore = &pro_symbol_guard; }                                                                              \
                                                                                                                                                      \
                private : static inline R pro_symbol_guard(Arg arg, P pq self) { return std::forward<Arg>(arg) __VA_ARGS__ static_cast<P pq>(self); } \
        )                                                                                                                                             \
    }
#define PROD_RHS_OP_DISPATCH_IMPL(...)                                                                                                                            \
    ...
    struct operator_dispatch<#__VA_ARGS__, true> {                                                                                                                \
        template<class T, class Arg>                                                                                                                              \
        PRO4D_STATIC_CALL(decltype(auto), T &&self, Arg &&arg)                                                                                                    \
        PRO4D_DIRECT_FUNC_IMPL(std::forward<Arg>(arg) __VA_ARGS__ std::forward<T>(self)) PRO4D_DEF_ACCESSOR_TEMPLATE(FREE, PROD_DEF_RHS_OP_ACCESSOR, __VA_ARGS__) \
    };
...
#define PROD_DEF_LHS_ASSIGNMENT_OP_ACCESSOR(oq, pq, ne, ...)                           \
    ...
    struct accessor<P, D, R(Arg) oq ne> {                                              \
        ...
        decltype(auto) __VA_ARGS__(Arg arg) oq ne {                                    \
            invoke<D, R(Arg) oq ne>(static_cast<P pq>(*this), std::forward<Arg>(arg)); \
            return static_cast<P pq>(*this);                                           \
        }                                                                              \
    }
...
#define PROD_ASSIGNMENT_OP_DISPATCH_IMPL(...)                                                                                                                                \
    ...
    struct operator_dispatch<#__VA_ARGS__, false> {                                                                                                                          \
        template<class T, class Arg>                                                                                                                                         \
        PRO4D_STATIC_CALL(decltype(auto), T &&self, Arg &&arg)                                                                                                               \
        PRO4D_DIRECT_FUNC_IMPL(std::forward<T>(self) __VA_ARGS__ std::forward<Arg>(arg)) PRO4D_DEF_ACCESSOR_TEMPLATE(                                                        \
                MEM,                                                                                                                                                         \
                PROD_DEF_LHS_ASSIGNMENT_OP_ACCESSOR,                                                                                                                         \
                operator __VA_ARGS__                                                                                                                                         \
        )                                                                                                                                                                    \
    };                                                                                                                                                                       \
    template<>                                                                                                                                                               \
    struct operator_dispatch<#__VA_ARGS__, true> {                                                                                                                           \
        template<class T, class Arg>                                                                                                                                         \
        PRO4D_STATIC_CALL(decltype(auto), T &&self, Arg &&arg)                                                                                                               \
        PRO4D_DIRECT_FUNC_IMPL(std::forward<Arg>(arg) __VA_ARGS__ std::forward<T>(self)) PRO4D_DEF_ACCESSOR_TEMPLATE(FREE, PROD_DEF_RHS_ASSIGNMENT_OP_ACCESSOR, __VA_ARGS__) \
    };
...
struct operator_dispatch<"()", false> {
    ...
    PRO4D_DIRECT_FUNC_IMPL(std::forward<T>(self)(std::forward<Args>(args)...)) PRO4D_DEF_ACCESSOR_TEMPLATE(MEM, PRO4D_DEF_MEM_ACCESSOR, operator())
};
template<>
struct operator_dispatch<"[]", false> {
...
struct view_conversion_dispatch: cast_dispatch_base<false, true> {
    ...
    {
        return observer_ptr<decltype(*value), decltype(*std::as_const(value)), decltype(*std::move(value)), decltype(*std::move(std::as_const(value)))>{*value};
    }
};
template<class F>
...
template<template<class...> class Formatter, template<class...> class StringView, template<class...> class ParseContext, template<class...> class FormatContext>
struct format_traits {
    template<class CharT>
    using overload = typename FormatContext<CharT>::iterator(
            StringView<CharT> spec,
            FormatContext<CharT> &fc
    ) const;

    struct dispatch {
        template<class T, class CharT>
        PRO4D_STATIC_CALL(auto, const T &self, StringView<CharT> spec, FormatContext<CharT> &fc)
...
struct proxy_cast_accessor_impl {
    ...
    friend T proxy_cast(Self self) {
        ...
        if constexpr(std::is_lvalue_reference_v<T>) {
            ...
            void *result = nullptr;
            proxy_cast_context ctx{.type_ptr = &typeid(T), .is_ref = true, .is_const = std::is_const_v<U>, .result_ptr = &result};
            ...
            proxy_cast_context ctx{.type_ptr = &typeid(T), .is_ref = false, .is_const = false, .result_ptr = &result};
            invoke<D, O>(static_cast<Self>(self), ctx);
    ...
    {
        void *result = nullptr;
        proxy_cast_context ctx{.type_ptr = &typeid(T), .is_ref = true, .is_const = std::is_const_v<T>, .result_ptr = &result};
        invoke<D, O>(*self, ctx);
...
template<class T, class CharT>
    requires(details::enabled_for<T, std::formatter, CharT>)
