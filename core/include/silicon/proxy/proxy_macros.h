...
#define PRO4D_DEF_ACCESSOR_TEMPLATE(type, macro, ...)                                                      \
    ...
    template<class ProP, class ProD, class... ProOs>                                                       \
        requires(sizeof...(ProOs) > 1u && (::std::is_constructible_v<accessor<ProP, ProD, ProOs>> && ...)) \
...
#define PRO4D_DEF_MEM_DISPATCH_IMPL(name, impl, func)                                                                                                                    \
    struct name {                                                                                                                                                        \
        ...
        PRO4D_DIRECT_FUNC_IMPL(::std::forward<ProT>(pro_self).impl(::std::forward<ProArgs>(pro_args)...)) PRO4D_DEF_ACCESSOR_TEMPLATE(MEM, PRO4D_DEF_MEM_ACCESSOR, func) \
    }
    ...
    struct accessor<ProP, ProD, ProR(ProArgs...) oq ne> {                                                                                                                                          \
        ...
        PRO4D_DEBUG(                                                                                                                                                                               \
                accessor() noexcept { ::std::ignore = &pro_symbol_guard; }                                                                                                                         \
                                                                                                                                                                                                   \
                private : static inline ProR pro_symbol_guard(ProP pq pro_self, ProArgs... pro_args) { return __VA_ARGS__(static_cast<ProP pq>(pro_self), ::std::forward<ProArgs>(pro_args)...); } \
        )                                                                                                                                                                                          \
    }
#define PRO4D_DEF_FREE_DISPATCH_IMPL(name, impl, func)                                                                                                                      \
    struct name {                                                                                                                                                           \
        template<class ProT, class... ProArgs>                                                                                                                              \
        PRO4D_STATIC_CALL(decltype(auto), ProT &&pro_self, ProArgs &&...pro_args)                                                                                           \
        PRO4D_DIRECT_FUNC_IMPL(impl(::std::forward<ProT>(pro_self), ::std::forward<ProArgs>(pro_args)...)) PRO4D_DEF_ACCESSOR_TEMPLATE(FREE, PRO4D_DEF_FREE_ACCESSOR, func) \
    }
...
#define PRO4D_DEF_FREE_AS_MEM_DISPATCH_IMPL(name, impl, func)                                                                                                             \
    struct name {                                                                                                                                                         \
        template<class ProT, class... ProArgs>                                                                                                                            \
        PRO4D_STATIC_CALL(decltype(auto), ProT &&pro_self, ProArgs &&...pro_args)                                                                                         \
        PRO4D_DIRECT_FUNC_IMPL(impl(::std::forward<ProT>(pro_self), ::std::forward<ProArgs>(pro_args)...)) PRO4D_DEF_ACCESSOR_TEMPLATE(MEM, PRO4D_DEF_MEM_ACCESSOR, func) \
    }
...
#    define __msft_lib_proxy                                                              \
        [] {                                                                              \
            PRO4D_AMBIGUOUS_MACRO_DIAGNOSTIC_ASSERT(__msft_lib_proxy, __msft_lib_proxy4); \
            return 0L;                                                                    \
        }()
#else
...
#    define PRO_DEF_FREE_DISPATCH(...) \
        PRO4D_AMBIGUOUS_MACRO_DIAGNOSTIC_ASSERT(PRO_DEF_FREE_DISPATCH, PRO4_DEF_FREE_DISPATCH)
#else
...
#    define PRO_DEF_FREE_AS_MEM_DISPATCH(...) \
        PRO4D_AMBIGUOUS_MACRO_DIAGNOSTIC_ASSERT(PRO_DEF_FREE_AS_MEM_DISPATCH, PRO4_DEF_FREE_AS_MEM_DISPATCH)
#else
