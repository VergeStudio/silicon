...
DOCTEST_NOINLINE bool unary_assert(assertType::Enum at, const char *file, int line, const char *expr, const DOCTEST_REF_WRAP(L) val) {
    ...
    if(at & assertType::Enum::is_false)
        failed = !failed;
...
const char *failureString(assertType::Enum at) {
    if(at & assertType::Enum::is_warn)
        return "WARNING";
    if(at & assertType::Enum::is_check)
        return "ERROR";
    if(at & assertType::Enum::is_require)
        return "FATAL ERROR";
    return "";
}
...
void addAssert(assertType::Enum at) {
    if((at & assertType::Enum::is_warn) == 0)
...
bool ResultBuilder::log() {
    if(m_at & assertType::Enum::is_throws) {
        m_failed = !m_threw;
    } else if((m_at & assertType::Enum::is_throws_as) && (m_at & assertType::Enum::is_throws_with)) {
        m_failed = !m_threw_as || !m_exception_string.check(m_exception);
    } else if(m_at & assertType::Enum::is_throws_as) {
        m_failed = !m_threw_as;
    } else if(m_at & assertType::Enum::is_throws_with) {
        m_failed = !m_exception_string.check(m_exception);
    } else if(m_at & assertType::Enum::is_nothrow) {
        m_failed = m_threw;
    }
...
void MessageBuilder::react() {
    if(m_severity & assertType::Enum::is_require)
        throwException();
}
}
namespace {
...
void fulltext_log_assert_to_stream(std::ostream &s, const AssertData &rb) {
    ...
    if(rb.m_at & assertType::Enum::is_throws) {
        s << (rb.m_threw ? "threw as expected!" : "did NOT throw at all!") << "\n";
    } else if((rb.m_at & assertType::Enum::is_throws_as) && (rb.m_at & assertType::Enum::is_throws_with)) {
        s << Color::Enum::Cyan << assertString(rb.m_at) << "( " << rb.m_expr << ", \""
    ...
    } else if(rb.m_at & assertType::Enum::is_throws_as) {
        s << Color::Enum::Cyan << assertString(rb.m_at) << "( " << rb.m_expr << ", "
          << rb.m_exception_type << " ) " << Color::Enum::None
...
#        endif
    ...
    void log_message(const MessageData &mb) override {
        if(mb.m_severity & assertType::Enum::is_warn)
            return;
