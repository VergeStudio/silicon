...
int main (void)
{
  ...
  sffi_type ts12_type,ts12b_type, ts12a_type;
  sffi_type *ts12_type_elements[4];
  sffi_type *ts12b_type_elements[3];
  sffi_type *ts12a_type_elements[3];
  ...
  ts12a_type.type = SFFI_TYPE_STRUCT;
  ts12a_type.elements = ts12a_type_elements;
  ts12a_type_elements[0] = &sffi_type_uchar;
  ts12a_type_elements[1] = &sffi_type_double;
  ts12a_type_elements[2] = NULL;

  ts12b_type.size = 0;
  ts12b_type.alignment = 0;
  ts12b_type.type = SFFI_TYPE_STRUCT;
  ts12b_type.elements = ts12b_type_elements;
  ts12b_type_elements[0] = &sffi_type_double;
  ts12b_type_elements[1] = &sffi_type_double;
  ts12b_type_elements[2] = NULL;

  ts12_type.size = 0;
  ts12_type.alignment = 0;
  ts12_type.type = SFFI_TYPE_STRUCT;
  ...
  CHECK(sffi_prep_cif(&cif, ABI_NUM, 2, &ts12_type, args) == SFFI_OK);
  ...
  sffi_call(&cif, SFFI_FN(struct12), ts12_result, values);
