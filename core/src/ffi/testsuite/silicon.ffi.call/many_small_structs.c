...
int main (void)
{
  ...
  ss_type.size = 0;
  ss_type.alignment = 0;
  ss_type.type = SFFI_TYPE_STRUCT;
  ss_type.elements = ss_elements;
  ss_elements[0] = &sffi_type_sint;
  ss_elements[1] = &sffi_type_sint;
  ss_elements[2] = NULL;
  ...
  CHECK(sffi_prep_cif(&cif, ABI_NUM, NARGS, &ss_type, args) == SFFI_OK);

  sffi_call(&cif, SFFI_FN(many_small), &result, values);
