...
void sffi_call_SYSV (extended_cif *,
		    unsigned, unsigned,
		    void *, void (*fn) ());
...
sffi_prep_args (void *stack, extended_cif *ecif)
{
  ...
  if (
#ifdef __MINT__
      (ecif->cif->rtype->type == SFFI_TYPE_LONGDOUBLE) ||
#endif
...
sffi_prep_cif_machdep (sffi_cif *cif)
{
  ...
  switch (cif->rtype->type)
    ...
    case SFFI_TYPE_STRUCT:
      if (cif->rtype->elements[0]->type == SFFI_TYPE_STRUCT &&
...
sffi_call (sffi_cif *cif, void (*fn) (), void *rvalue, void **avalue)
{
  ...
  switch (cif->abi)
    {
    case SFFI_SYSV:
      sffi_call_SYSV (&ecif, cif->bytes, cif->flags,
		     ecif.rvalue, fn);
      break;
...
static void
sffi_prep_incoming_args_SYSV (char *stack, void **avalue, sffi_cif *cif)
{
  unsigned int i;
  void **p_argv;
  char *argp;
...
unsigned int
sffi_closure_SYSV_inner (sffi_closure *closure, void *resp, void *args)
{
  sffi_cif *cif;
  void **arg_area;
  ...
  sffi_prep_incoming_args_SYSV(args, arg_area, cif);
		      ...
		      void *codeloc)
{
  ...
  *(unsigned short *)(closure->tramp + 6) = 0x4ef9;
  ...
  if (
      ...
      (((cif->rtype->type == SFFI_TYPE_STRUCT)
         && !cif->flags)))
    *(void **)(closure->tramp + 8) = sffi_closure_struct_SYSV;
  else
    *(void **)(closure->tramp + 8) = sffi_closure_SYSV;

#ifdef __rtems__
  rtems_cache_flush_multiple_data_lines( codeloc, SFFI_TRAMPOLINE_SIZE );
#elif defined(__MINT__)
  Ssystem(S_FLUSHCACHE, codeloc, SFFI_TRAMPOLINE_SIZE);
#else
  syscall(SYS_cacheflush, codeloc, FLUSH_SCOPE_LINE,
	  FLUSH_CACHE_BOTH, SFFI_TRAMPOLINE_SIZE);
#endif
